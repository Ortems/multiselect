...
(function ($) {
	...
	$.widget("ui.multiselect", {
		...
		_create: function () {
			...
			this.limitReachedMessage = $('<div style="color: #ff0000; display: none; text-align:center;">' + $.ui.multiselect.locale.itemsLimitReached + '</div>').appendTo(this.container);
		...
		_batchSelect: function (elements, state) {
			...
			if (!state) that._filter.apply(that.availableContainer.find('input.search'), [that.availableList]); ;
...
.keyup(function () {
	...
	$.extend($.ui.multiselect, {
		locale: {
			...
			itemsLimitReached: 'Items limit reached'
		}
	});
